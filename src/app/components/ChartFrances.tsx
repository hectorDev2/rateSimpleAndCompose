...
const ChartFrances = ({ pmt, capital, rate, time }: any) => {
  // Generar datos para cada período de tiempo
  const labels = Array.from({ length: time }, (_, i) => i + 1);
  const capitalData = labels.map((t) => pmt - rate);
  console.log(Math.round((pmt - rate + Number.EPSILON) * 100) / 100);
